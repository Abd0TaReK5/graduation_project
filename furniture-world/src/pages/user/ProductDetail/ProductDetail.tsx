...
export const ProductDetailsPage = () => {
    ...
    const productDetail = products.find(p => p.id.toString() === id);
        ...
        >
            ...
            >
                ...
                <Row style={{ gap: '20px', width: '50%', display:'flex', justifyContent: 'center' }}>
                  ...
                  {!productDetail ? (
    <Text style={{ marginTop: 100 }}>المنتج غير موجود</Text>
) : (
    <Image
        alt={productDetail.name || 'Product image'}
        width={300}
        height={300}
        src={productDetail.image_dir || assets.defaultProductImage}
    />
)}
                </Row>
                <Flex style={{ flexDirection: 'column', width: '50%' }}>
                    ...
                    >
                        {productDetail && exchangeRate ? (
                        <Text style={{ fontSize: '18px', fontWeight: '500' ,color: token.colorPrimary }}>
                            {Math.round(productDetail.price * exchangeRate)} EGP
                        </Text>
                        ) : (
