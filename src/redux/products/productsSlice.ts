...
try {
    const productImage = await getProductImageAPI(product.id); // استدعاء API لجلب الصورة
    if (productImage.status === 200) {
        // تحديد الرابط المباشر للصورة
        productImageUrl = `http://localhost:5000/api/productimg/getProductPicture?id=${product.id}`;
    } else {
        throw new Error('Failed to fetch image');
    }
} catch (error) {
    console.error('Error fetching product image:', error);
    productImageUrl = '/images/default-image.jpg'; // هنا نعرض صورة افتراضية في حالة الخطأ
}
